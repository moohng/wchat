<template>
  <footer class="chat-bar">
    <div class="content">
      <div class="wrap">
        <div class="icon">
          <icon-voice></icon-voice>
        </div>
      </div>
      <div class="wrap text-area">
        <p class="edit-box" contenteditable ref="input"
        @keydown.enter.prevent="send($event.target)"
        @input.prevent="input($event.target)"></p>
      </div>
      <div class="wrap">
        <div class="icon">
      ...
      <div class="wrap">
        <transition name="scale" mode="out-in">
          <a class="icon send-btn" v-if="showSend"
          @click.stop.prevent="send($refs['input'])">发送</a>
